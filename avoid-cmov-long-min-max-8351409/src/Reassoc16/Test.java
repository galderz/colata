...
final class Test {
  ...
  static long test(long[] array) {
    ...
    for (int i = 0; i < array.length; i += 16) {
      ...
      var t7 = Math.max(v8, t6);
      var t8 = Math.max(v9, t7);
      ...
      // result = max(result, max(v15, max(v14, max(v13, max(v12, max(v11, max(v10, max(v9, max(v8, max(v7, max(v6, max(v5, max(v4, max(v3, max(v2, max(v1, v0))))))))))))))))
