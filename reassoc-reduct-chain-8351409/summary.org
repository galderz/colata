...
* Next steps
- [ ] Test HF reassociation
- [X] call igvn.replace_node on reassociated node
- [X] check the assembly and make sure the differences in IR are reflected there
- [X] check the performance of manual unroll 16 and compare with prototype
- [X] try to understand why manual reassoc4 is faster than manual reassoc16 (investigate unrolling in reassoc4, disable loop unrolling)
- [X] generate reassoc2x8
- [X] generate reassoc8x2
- [X] generate binary tree reassoc
- [X] remove striped counted loop check, not necessary
- [X] try to get it working with a manually unrolled loops where one of the intermediate values is somehow either inside or outside the loop
      e.g. if statement for an intermediate value, or just use it outside
- [ ] add a todo/question on the possibility of having the chain via the second input (check AddNode::commute and see what that does)
- [ ] find a benchmark that would showcase this scenario
- [X] see if an IR test can be developed, though IR node shape is not a feature
* 22.1.26
