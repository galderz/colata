...
- [ ] generate binary tree reassoc
- [ ] remove striped counted loop check, not necessary
- [ ] try to get it working with a manually unrolled loops where one of the intermediate values is somehow either inside or outside the loop
...
- [ ] see if an IR test can be developed, though IR node shape is not a feature
* 04.12.25
